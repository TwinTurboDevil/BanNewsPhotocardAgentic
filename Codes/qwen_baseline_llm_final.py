...
OUTPUT_CSV = "G:/News Photocard Research Thesis/qwen_baseline_result.csv"
LOG_FILE = "G:/News Photocard Research Thesis/qwen_baseline_run.log"

MODEL_NAME = "qwen2.5vl:7b"

CSV_FIELDS = ["image_name", "actual", "prediction", "confidence", "reason"]

# Safety limit -- see process_single_photocard() for why this was added
MAX_OUTPUT_TOKENS = 512       # a short JSON verdict never needs more than this
...
log = logging.getLogger("qwen_baseline")
...
def process_single_photocard(img_path, img_name, actual_label):
  """Runs one image through the baseline model. Returns a result dict.

  Safety net: num_predict caps how many tokens the model is ALLOWED to
  generate -- a short JSON verdict never needs more than a few hundred
  tokens, which guards against a runaway/repetition generation loop.
  (Timeout/force-unload handling removed for this PC -- not needed here.)
  """
  try:
    response = ollama.chat(
        ...
        options={
            "num_ctx": 8192,                    # Context window set to 8192 tokens
            "num_predict": MAX_OUTPUT_TOKENS,    # cap generation length -- prevents runaway loops
        },
    )
    ...
    try:
      ...
      # Fallback regex extraction if strict JSON parsing fails due to unescaped characters/quotes
...
def run_open_source_baseline():
  ...
  log.info(
      f"Starting Open-Source Vision LLM ({MODEL_NAME}) Baseline on {len(remaining)} images..."
  )
  ...
  # checkpoint: if the process is interrupted (crash, Ctrl+C, PC restart),
  ...
  with open(OUTPUT_CSV, "a", encoding="utf-8-sig", newline="") as f:
    ...
    for img_path, img_name, actual_label in tqdm(remaining, desc="Processing Photocards"):
      res = process_single_photocard(img_path, img_name, actual_label)
  ...
  log.info("\n" + "=" * 50)
  log.info("OPEN-SOURCE BASELINE EVALUATION SUMMARY (this run)")
  log.info("=" * 50)
